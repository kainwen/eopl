...
type_of_proc({proc_exp, Paras_with_type, Body}, Tenv) ->
    ...
    {arrow,
     inferred_type:reduce_type({star, [Tp || {_, Tp} <- Paras_with_type]}),
     Body_type}.

type_of_apply(Exp={apply_exp, Operator, Operands}, Tenv) ->
    T_operator = type_of(Operator, Tenv),
    T_operands = inferred_type:reduce_type({star, [type_of(Op, Tenv) || Op <- Operands]}),
    case T_operator of
        {arrow, T1, T2} ->
            assert_arg_num(inferred_type:card(T1), length(Operands), Exp),
...
type_of_letrec(Exp={letrec_exp, Fns_with_return_type, Proc_defs, Body}, Tenv) ->
    Proc_input_types = [inferred_type:reduce_type({star,
