...
-spec unifier(type_with_var(), type_with_var(), subst(), exp()) -> subst().
unifier(Tv1, T2, Subst, Exp) when is_atom(Tv1)->
...
unifier(T1, Tv2, Subst, Exp) when is_atom(Tv2)->
...
unifier({int}, {int}, Subst, _E) -> Subst;
unifier({int}, T2, _Subst, Exp) ->
    erlang:error({unifier_fail, {int}, T2, Exp});
unifier({bool}, {bool}, Subst, _E) -> Subst;
unifier({bool}, T2, _Subst, Exp) ->
    erlang:error({unifier_fail, {bool}, T2, Exp});
unifier({arrow, T11, T12}, {arrow, T21, T22}, Subst, Exp) ->
    S1 = unifier(T11, T21, Subst, Exp),
    unifier(T12, T22, S1, Exp);
unifier(T1={arrow, _, _}, T2, _S, Exp) ->
    erlang:error({unifier_fail, T1, T2, Exp});
unifier({star, Tps1}, {star, Tps2}, Subst, Exp) ->
    unifier_list(Tps1, Tps2, Subst, Exp);
unifier(T1={star, _}, T2, _Subst, Exp) ->
    erlang:error({unifier_fail, T1, T2, Exp});
unifier({list, T1}, {list, T2}, Subst, Exp) ->
    unifier(T1, T2, Subst, Exp);
unifier(T1={list, _}, T2, _Subst, Exp) ->
    erlang:error({unifier_fail, T1, T2, Exp}).
