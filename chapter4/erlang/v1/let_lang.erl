...
eval_abt({var_exp, Id}, Env, _Store) -> env:apply_env(Env, Id);
...
eval_abt({let_exp, Bindlist, Body}, Env, Store) ->
    Binding_pairs = [{Id, eval_abt(E, Env, Store)}
...
eval_abt({'letrec_exp', Proc_binding_list, Body}, Env, Store) ->
    ...
    eval_abt(Body, New_env, Store);
eval_abt({newref_exp, Exp}, Env, Store) ->
    Val = eval_abt(Exp, Env, Store),
    Ref = store:newref(Store, Val),
    {ref_val, Ref};
eval_abt({deref_exp, Exp}, Env, Store) ->
    {ref_val, Ref} = eval_abt(Exp, Env, Store),
    store:deref(Store, Ref);
eval_abt({setref_exp, Exp1, Exp2}, Env, Store) ->
    {ref_val, Ref} = eval_abt(Exp1, Env, Store),
    Val = eval_abt(Exp2, Env, Store),
    {num_val, store:setref(Store, Ref, Val)};
