...
-type abstract_syntax_tree() :: {number_exp, integer()}
                              ...
                              | {newref_exp, abstract_syntax_tree()}
                              | {deref_exp, abstract_syntax_tree()}
                              | {setref_exp, abstract_syntax_tree(), abstract_syntax_tree()}
...
parse_exp(Tks=['letrec'|_Rem_toks]) ->
    parse_letrec(Tks);
parse_exp(Tks=['newref'|_Rem_toks]) ->
    parse_newref(Tks);
parse_exp(Tks=['deref'|_Rem_toks]) ->
    parse_deref(Tks);
parse_exp(Tks=['setref'|_Rem_toks]) ->
    parse_setref(Tks);
...
parse_newref(Tks) ->
    R1 = wait_for('(', wait_for('newref', Tks)),
    {Exp, R2} = parse_exp(R1),
    R3 = wait_for(')', R2),
    {{newref_exp, Exp}, R3}.

parse_deref(Tks) ->
    R1 = wait_for('(', wait_for('deref', Tks)),
    {Exp, R2} = parse_exp(R1),
    R3 = wait_for(')', R2),
    {{deref_exp, Exp}, R3}.

parse_setref(Tks) ->
    R1 = wait_for('(', wait_for('setref', Tks)),
    {Exp1, R2} = parse_exp(R1),
    R3 = wait_for(',', R2),
    {Exp2, R4} = parse_exp(R3),
    R5 = wait_for(')', R4),
    {{setref_exp, Exp1, Exp2}, R5}.
