...
eval_abt({setdynamic_exp, {var_exp, Var}, Exp, Body}, Env, Store) ->
    Val = eval_abt(Exp, Env, Store),
    Ref = store:newref(Store, Val),
    eval_abt(Body, env:extend_env_with(Var, Ref, Env), Store).

%% internal help function
get_proc({proc_val, {Paras, Body, Lex_env}}, {extend_env_with, Var, Val, _Saved_env}) ->
    {proc_val, {Paras, Body, env:extend_env(Var, Val, Lex_env)}};
...
eval_test() ->
    [
     ...
     eval_script("code/code5", store6) =:= {num_val, 3}
    ].
