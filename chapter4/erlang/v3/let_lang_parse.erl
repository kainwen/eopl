...
-type abstract_syntax_tree() :: {number_exp, integer()}
                              ...
                              | {setdynamic, var(), abstract_syntax_tree(), abstract_syntax_tree()}.
...
parse_exp(Tks=['set'|_Rem_toks]) ->
    parse_assign(Tks);
parse_exp(Tks=['setdynamic'|_Rem_toks]) ->
    parse_setdynamic(Tks).
...
parse_setdynamic(Tks) ->
    R1 = wait_for('setdynamic', Tks),
    {Var, R2} = parse_var(R1),
    R3 = wait_for('=', R2),
    {Exp, R4} = parse_exp(R3),
    R5 = wait_for('during', R4),
    {Body, R6} = parse_exp(R5),
    {{setdynamic_exp, Var, Exp, Body}, R6}.
