...
-type keywords() :: 'if' | 'then' | 'else' | 'zero?' | 'let' | '=' | 'in'
                  ...
                  | 'end' | ';'  | 'newarray' | 'arrayref' | 'arrayset'
                  | '-' | '(' | ')' | ','.
...
-type abstract_syntax_tree() :: {number_exp, integer()}
                              ...
                              | {newarray_exp, abstract_syntax_tree(), abstract_syntax_tree()}
                              | {arrayref_exp, abstract_syntax_tree(), abstract_syntax_tree()}
                              | {arrayset_exp, abstract_syntax_tree(), abstract_syntax_tree(), abstract_syntax_tree()}
                              | {arraylength_exp, abstract_syntax_tree()}.
...
parse_exp(Tks=['set'|_Rem_toks]) ->
    parse_assign(Tks);
parse_exp(Tks=['newarray'|_Rem_toks]) ->
    parse_newarray(Tks);
parse_exp(Tks=['arrayref'|_Rem_toks]) ->
    parse_arrayref(Tks);
parse_exp(Tks=['arrayset'|_Rem_toks]) ->
    parse_arrayset(Tks);
parse_exp(Tks=['arraylength'|_Rem_toks]) ->
    parse_arr_len(Tks).


%% internal parse helper functions
parse_arr_len(Tks) ->
    R1 = wait_for('(', wait_for('arraylength', Tks)),
    {Arr_exp, R2} = parse_exp(R1),
    {{arraylength_exp, Arr_exp}, R2}.

parse_arrayset(Tks) ->
    R1 = wait_for('(', wait_for('arrayset', Tks)),
    {Arr_exp, R2} = parse_exp(R1),
    R3 = wait_for(',', R2),
    {Index_exp, R4} = parse_exp(R3),
    R5 = wait_for(',', R4),
    {Val_exp, R6} = parse_exp(R5),
    R7 = wait_for(')', R6),
    {{arrayset_exp, Arr_exp, Index_exp, Val_exp}, R7}.

parse_arrayref(Tks) ->
    R1 = wait_for('(', wait_for('arrayref', Tks)),
    {Arr_exp, R2} = parse_exp(R1),
    R3 = wait_for(',', R2),
    {Index_exp, R4} = parse_exp(R3),
    R5 = wait_for(')', R4),
    {{arrayref_exp, Arr_exp, Index_exp}, R5}.

parse_newarray(Tks) ->
    R1 = wait_for('(', wait_for('newarray', Tks)),
    {Len_exp, R2} = parse_exp(R1),
    R3 = wait_for(',', R2),
    {Val_exp, R4} = parse_exp(R3),
    R5 = wait_for(')', R4),
    {{newarray_exp, Len_exp, Val_exp}, R5}.
