...
eval_abt({'apply_exp', Operator, Operands}, Env, Store) ->
    {proc_val, {Vars, Body, Lex_env}} = get_proc(eval_abt(Operator, Env, Store),
                                                 Env),
    Args = [eval_parameter(Operand, Env, Store) || Operand <- Operands],
    New_env = env:extend_env_by_list(Lex_env, lists:zip(Vars, Args)),
    eval_abt(Body, New_env, Store);
...
eval_parameter({var_exp, Var}, Env, _Store) ->
    env:apply_env(Env, Var);
eval_parameter(Operand, Env, Store) ->
    Val = eval_abt(Operand, Env, Store),
...
eval_test() ->
    [
     ...
     eval_script("code/code4", store5) =:= {num_val, 11}
    ].
