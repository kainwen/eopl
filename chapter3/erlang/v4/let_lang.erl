...
eval_abt({unpack_exp, Vars, Exp, Body}, Env) ->
    ...
    eval_abt(Body, New_env);
eval_abt({'proc_exp', Paras, Body}, Env) ->
    {proc_val, {[Id || {var_exp, Id} <- Paras], Body, Env}};
eval_abt({'apply_exp', Operator, Operands}, Env) ->
    {proc_val, {Vars, Body, Lex_env}} = eval_abt(Operator, Env),
    Vals = [eval_abt(E, Env) || E <- Operands],
    Pairs = lists:zip(Vars, Vals),
    New_env = env:extend_env_by_list(Lex_env, Pairs),
    eval_abt(Body, New_env);
eval_abt({'letrec_exp', {var_exp, Proc_name}, Paras, Proc_body, Body}, Env) ->
    New_env = env:extend_env_rec(Proc_name, [P || {var_exp, P} <- Paras], Proc_body, Env),
    eval_abt(Body, New_env).
