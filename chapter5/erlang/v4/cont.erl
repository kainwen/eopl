...
-export([
         ...
         cons1_cont/3, test_null_cont/1, invoke1_cont/3, throw1_cont/3
        ]).
...
-type cont_element() :: {end_cont, Hanlder_pos::integer(), {}}
                      ...
                      | {throw1_cont, Handler_pos::integer(), {Exp::exp(), Env::env()}}
                      | {throw2_cont, Handler_pos::integer(), {Val::val()}}.
...
-spec throw1_cont(cont(), exp(), env()) -> cont().
throw1_cont(Cont, Exp, Env) ->
    Handler_pos = get_handler_pos(Cont),
    Cont_element = {throw1_cont, Handler_pos, {Exp, Env}},
    push_cont(Cont_element, Cont).

-spec throw2_cont(cont(), val()) -> cont().
throw2_cont(Cont, Val) ->
    Handler_pos = get_handler_pos(Cont),
    Cont_element = {throw2_cont, Handler_pos, {Val}},
    push_cont(Cont_element, Cont).
...
apply_cont(Cont, Val) ->
    ...
    case Cont_element of
        ...
        {invoke2_cont, _Handler_pos, {{cont_val, Ct}}} ->
            apply_cont(Ct, Val);
        {throw1_cont, _Handler_pos, {Exp, Env}} ->
            Saved_cont = array:resize(Len-1, Cont),
            New_cont = throw2_cont(Saved_cont, Val),
            let_lang:eval(Exp, Env, New_cont);
        {throw2_cont, _Handler_pos, {Val1}} ->
            {cont_val, Ct} = Val,
            apply_cont(Ct, Val1)
    end.
